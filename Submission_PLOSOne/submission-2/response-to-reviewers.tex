...
subsection of paper-plos.tex. The prior values above are VERIFIED against the canonical
pipeline (\texttt{supp-4-lik-comparison/02-array-run-model.R} line 63, inside the Poisson
branch: \texttt{matern.pri.feat.count <- c(500, .95, .1, .05)} passed to
\texttt{inla.spde2.pcmatern} as \texttt{prior.range = c(500, 0.95)},
\texttt{prior.sigma = c(0.1, 0.05)}, with \texttt{alpha = 2} and \texttt{constr = TRUE}).
Note the file-level default at line 32 is \texttt{c(500, .95, 1, .05)}, with sigma 1 rather
than 0.1; it is overridden inside the Poisson branch, so quote the line-63 values. Posterior
summaries use \texttt{n.samp <- 1000} draws.}
...
\todo{Still needs the actual justification for Poisson over ZIP and ZAP written against the
Table 1 numbers, including the metrics that disagree (MDS favours ZAP at 82\% and ZIP at
85\%, while MAE, RMSE and MLG favour Poisson), and the metric definitions written as
equations in the Methods.
Two things about Table 1 are now verified from \texttt{03-array-post-process.R} and can be
stated directly. (i) The parenthetical percentages are the mean over features of the ratio
of that model's score to the Poisson score \emph{for the same feature}, expressed as a
percentage, so 100\% means indistinguishable from Poisson. They are NOT the ratio of the two
averages printed in the table, which is why 0.29 against 0.21 shows as 111\% rather than
138\%. The current caption calls this an ``average percentage difference,'' which is a
misnomer for a ratio and should be reworded. (ii) The averages exclude features with
abnormal values indicating nonconvergence, via the filter \texttt{MAE < 100 \&
abs(MDS) < 100}, which the code comments say drops 4 and 3 features respectively. The
caption's ``all 918 modeled ligand/receptor features'' is wrong on both counts and needs
restating.}
...
of the Results and a new supplementary figure, and the code and summary outputs are in the
...
list. On inspection, the 1481-column object used for the high-dimensional clustering
contains four pairs of exactly duplicated columns, which we traced to files that had been
silently duplicated by a cloud file-synchronization conflict and then read in alongside their
originals. The four affected interactions are PDGFC-PDGFRA, PDGFC-PDGFRB, PDGFD-PDGFRA, and
PDGFD-PDGFRB; in each case the duplicate column is numerically identical to its counterpart.
The correct number of distinct interaction fields computable from the 912 fitted features is
therefore 1477. We have corrected the count wherever it appears, including the upper limit of
the feature-depth sweep in Figure 6.
...
\todo{Apply these corrections to every occurrence in the manuscript and captions: 918 to 912
at line 217; 1481 to 1477 at lines 213 and 251 and in the Fig 6 caption; and the Table 1
caption, which averages over the fitted features that pass the score filter, not over ``all
918 modeled ligand/receptor features.'' Decide with Raredon whether to name the
synchronization conflict as the cause in the manuscript or to describe it more neutrally as
