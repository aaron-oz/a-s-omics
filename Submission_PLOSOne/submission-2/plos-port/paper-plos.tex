...
50 across the 49 coordinate-permuted replicates, each refit from scratch, with the observed
value marked in red. Lower ELSA means more spatially coherent domains, so the observed data
sit far below the null. \textbf{(B)} The same comparison for the number of domains recovered:
20 in the observed data against 4 to 12 across the permutations. \textbf{(C)} The emergent
signalling domains (left) beside the anatomical annotation published for the same tissue
section (right), on identical axes with square bins. Colours are arbitrary labels in both
panels, so what is comparable is the layout of the regions, not the colour matching.
\textbf{(D)} Concordance between the two partitions in (C), measured by adjusted Rand index,
against a null from 499 random permutations of the domain labels. All 499 fall below 0.0011,
against an observed 0.401. Generated by
